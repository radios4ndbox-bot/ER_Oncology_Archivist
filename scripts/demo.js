...
/* Demo dimostrativa: avvia l'app con l'archivio di pazienti INVENTATI
   conservato in demo/ER OA Archive.json.
...
const SORGENTE = path.join(ROOT, 'demo', 'ER OA Archive.json');
const CARTELLA = path.join(ROOT, 'dev-data');
const DESTINAZIONE = path.join(CARTELLA, 'ER OA Archive.json');
...
if (fs.existsSync(DESTINAZIONE)) {
  ...
  const copia = path.join(CARTELLA, 'ER OA Archive.prima-della-demo-' + marca + '.json');
...
['ER OA Archive.lock', 'ps_onco.lock'].forEach((l) => {
  try { fs.unlinkSync(path.join(CARTELLA, l)); } catch (_) { /* nessun lock */ }
});
